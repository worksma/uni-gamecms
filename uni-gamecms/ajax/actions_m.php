...
if(isset($_POST['refill_balance'])) {
	...
	try {
		...
		$STH = $pdo->query("SELECT min_amount FROM config__secondary LIMIT 1");
		...
		switch($cashier['slug']) {
			case 'ik':
				$STH = $pdo->query("SELECT ik_login, ik_pass1 FROM config__bank LIMIT 1");
				$STH->setFetchMode(PDO::FETCH_OBJ);
			...
			case 'ya':
				...
				break;
			case 'fk':
				$STH = $pdo->query("SELECT fk_login, fk_pass1 FROM config__bank LIMIT 1");
				$STH->setFetchMode(PDO::FETCH_OBJ);
				$cashierSettings = $STH->fetch();
				...
				break;
			case 'amarapay':
				$sth = $pdo->query("SELECT `amarapay_id`, `amarapay_public` FROM `config__bank` LIMIT 1");
				$sth->setFetchMode(PDO::FETCH_OBJ);
				$cashierSettings = $sth->fetch();
				...
				$hash = hash('sha256', $cashierSettings->amarapay_id
				. $amount
				. $cashierSettings->amarapay_public
				. $_SESSION['id']);
			...
			case 'rb':
				...
				break;
			case 'wb':
				$STH = $pdo->query("SELECT wb_login, wb_num FROM config__bank LIMIT 1");
				$STH->setFetchMode(PDO::FETCH_OBJ);
				$cashierSettings = $STH->fetch();
				...
				break;
			case 'up':
				$STH = $pdo->query("SELECT up_pass1, up_pass2 FROM config__bank LIMIT 1");
				$STH->setFetchMode(PDO::FETCH_OBJ);
				$cashierSettings = $STH->fetch();
				...
				$unitPay = new UnitPay('unitpay.money', $cashierSettings->up_pass2);
				...
				break;
			case 'ps':
				$STH = $pdo->query("SELECT ps_pass, ps_num, ps_currency, ps_test FROM config__bank LIMIT 1");
				$STH->setFetchMode(PDO::FETCH_OBJ);
				$cashierSettings = $STH->fetch();
				...
				break;
			case 'wo':
				$STH = $pdo->query("SELECT wo_login, wo_pass FROM config__bank LIMIT 1");
				$STH->setFetchMode(PDO::FETCH_OBJ);
				$cashierSettings = $STH->fetch();
...
if(isset($_POST['buy_service'])) {
	...
	if($admin['type'] == 'ce' || $admin['type'] == 'ca') {
		$STH = $pdo->prepare("SELECT id,user_id,active,pause FROM admins WHERE name LIKE :name AND server=:server LIMIT 1");
		$adminNameForCheck = '%' . strip_data(':' . explode(':', $admin['name'])[2]);
	} else {
		$STH = $pdo->prepare("SELECT id,user_id,active,pause FROM admins WHERE name=:name AND server=:server LIMIT 1");
	...
	$STH = $pdo->query("SELECT id,pirce,time,discount FROM services__tarifs WHERE id='$tarif' and service='$service->id' LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$tarif = $STH->fetch();
	if(empty($tarif->id)) {
	...
	if($admin['has_rights'] == 1) {
		...
		if(!$STH->execute(array('name'     => $admin['name'],
								'pass'     => $admin['pass'],
	...
	if(!$STH->execute(array(':admin_id'       => $admin['id'],
							...
							':previous_group' => $admin['previous_group'])) == '1') {
...
if(isset($_POST['buy_unban'])) {
	...
	$STH = $pdo->query("SELECT id,db_host,db_user,db_pass,db_db,db_prefix,type,db_code FROM servers WHERE id='$server' LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$server = $STH->fetch();
	if(empty($server->id)) {
		exit(json_encode(array('status' => '2')));
	...
	if($server->type == '2' || $server->type == '3' || $server->type == '5') {
		...
		$STH    = $pdo2->query("SELECT $table1.bid,$table1.ip AS player_ip,$table1.RemoveType AS expired,$table1.authid AS player_id,$table1.name AS player_nick,$table1.created AS ban_created,$table1.length AS ban_length,$table1.reason AS ban_reason FROM $table1 WHERE $table1.bid = '$id'");
		$STH->setFetchMode(PDO::FETCH_OBJ);
		$ban = $STH->fetch();
	}
	if(empty($ban->bid)) {
...
if(isset($_POST['buy_unmute'])) {
	...
	$STH = $pdo->query("SELECT id,db_host,db_user,db_pass,db_db,db_prefix,type,db_code FROM servers WHERE id='$server' and type!=0  LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$serv_info = $STH->fetch();
	...
	if($type == '1' || $type == '2' || $type == '3' || $type == '5') {
		...
		$STH = $pdo->query("SELECT $table.bid,$table.expired,$table.authid AS player_id,$table.name AS player_nick,$table.created AS ban_created,$table.length AS ban_length,$table.reason AS ban_reason FROM $table WHERE $table.bid = '$id'");
		$STH->setFetchMode(PDO::FETCH_OBJ);
		$ban = $STH->fetch();
		...
		$STH   = $pdo2->query("SELECT $table.bid,$table.RemoveType AS expired,$table.authid AS player_id,$table.name AS player_nick,$table.created AS ban_created,$table.length AS ban_length,$table.reason AS ban_reason FROM $table WHERE $table.bid = '$id'");
		$STH->setFetchMode(PDO::FETCH_OBJ);
		$ban = $STH->fetch();
...
if(isset($_POST['get_stores_info'])) {
	...
	while($row = $STH->fetch()) {
		...
		if($admin->active != 2 and $disabled != "disabled" and $disabled == "" and $disp == "") {
			...
			$STH2     = $pdo->query("SELECT id,pirce,time,discount FROM services__tarifs WHERE service = '$row->service' ORDER BY pirce");
			$STH2->setFetchMode(PDO::FETCH_OBJ);
...
if(isset($_POST['edit_srote'])) {
	...
	if($type == 'type') {
		if($conf->col_type == 0) {
			exit (json_encode(array('status' => '2', 'reply' => 'Смена типа привязки запрещена')));
		}
		...
		if(!empty($row->id)) {
			...
			if($delta < (24 * 60 * 60 * $conf->col_type)) {
				exit (json_encode(array('status' => '2', 'reply' => 'Тип привязки разрешено менять раз в '.$conf->col_type.' дн.')));
			}
		}
	...
	if($type == 'name') {
		...
		if($admin->type == 'ce' or $admin->type == 'ca') {
			$STH = $pdo->prepare("SELECT id FROM admins WHERE name LIKE :name AND server=:server LIMIT 1");
...
if(isset($_POST['start_srote'])) {
	...
	try {
		$SQ->reolad_admins($pdo, $info->server_id, $admin_id);
	} catch(Exception $e) {
		$err = 1;
	}
	unset($SQ);
...
if(isset($_POST['buy_extend'])) {
	...
	$STH = $pdo->query("SELECT id,time,pirce,discount FROM services__tarifs WHERE id='$time' and service='$admin->service'");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$tarif = $STH->fetch();
	if(empty($tarif->id)) {
		exit(json_encode(array('status' => '2')));
