<tr id="admin{id}">
	...
	<td id="new_active_{id}">
		{if('{active}' == '2')}
			<font id="admin_block{id}" class="text-danger" data-container="body" data-toggle="popover" data-placement="top" title="Заблокирован" data-content='Причина: {cause}<br>Цена разблокировки: {pirce}{{$messages['RUB']}}{if('{link}'!='')}<br>Ссылка на <a target="blank" href="{link}">доказательства</a>{/if}'>
				Заблокирован
			</font>
